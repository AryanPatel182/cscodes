...
% Trapezoidal Rule
    ...
    for i=1:9
        ...
        for j=2:n(i)+1      
            ...
            if j<=n(i)
                ...
                int_of_f(i)=int_of_f(i)+y(j)/2;
            end
        end
        int_of_f(i) = h*int_of_f(i);
        % Asymptotic Error
        asymperr(i) = -((h^2)*(f_p(b)-f_p(a)))/12;
    end
    ...
    trapezoidal_table = table(n,int_of_f,Error,Ratio,asympratio,asymperr); % answer table
    disp('<strong> Answer by Trapezoid Rule</strong>');
    disp(trapezoidal_table);